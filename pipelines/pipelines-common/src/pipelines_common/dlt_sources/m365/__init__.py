...
from msgraphfs import MSGDriveFS, MSGraphBuffredFile
...
class M365CredentialsResource:
    ...
    def as_graph_credentials(self) -> GraphCredentials:
        return GraphCredentials(self.tenant_id, self.client_id, self.client_secret)
...
def sharepoint(
...
) -> Iterator[List[MSGraphBuffredFile]]:
    ...
    """
    graph_client = GraphClientV1(credentials=credentials.as_graph_credentials())
    site = graph_client.site(site_url)
    library_root = site.document_library()

    yield library_root.glob(file_glob)
